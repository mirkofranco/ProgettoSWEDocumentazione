...
L'integrazione continua avviene mediante l'azione congiunta di \glossario{Travis} e \glossario{SonarQube}:ad ogni modifica vengono eseguiti script automatici per il controllo della qualità del codice, il risultato di ogni build viene notificata ai membri del gruppo tramite il canale \glossario{Slack} dedicato.

\subsection{Test}
\'E compito dei \textit{Progettisti} definire i test nel periodo di progettazione,è opportuno inoltre stabilire quali test eseguire in modo automatico tramite \textit{Travis-CI}, e quali richiedono un'esecuzione manuale da parte di un Verificatore, vengono fornite inoltre ulteriori indicazioni sulla coretta codifica dei test e la loro natura, suddividendo per tecnologie adottate qualora le differenze richiedano un dettaglio maggiore.\\
...
\subsubsection{Codifica e esecuzione dei test}
	...
	Alternativamente, è possibile definire un test che utilizzi dei \glossario{Mock} degli oggetti creati dall'ambiente di sviluppo, tuttavia è altamente sconsigliato perchè richiederebbe una quantità di lavoro eccessivo e letture successive del codice sarebbero poco intuitive.\\
	...
	\begin{itemize}
			...
			\item Evitare l'utlizzo del comando \textit{retries()} del tutto.
		\end{itemize}
...
\begin{itemize}
	...
	\item  \textbf{com.megalexa.models} conterrà tutti i file riguardanti la modellazione delle classi;
	\item  \textbf{com.megalexa.ui} conterrà tutti i file riguardanti lo sviluppo dell'interfaccia grafica;
	\item  \textbf{com.megalexa.network} conterrà tutto il codice necessario per la comunicazione attraverso l'utilizzo della rete;
	\item  \textbf{com.megalexa.utils} conterrà tutte la classi di utilità definite nel corso dello sviluppo;	
