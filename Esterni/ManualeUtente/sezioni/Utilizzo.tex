...
\textbf{NOTA BENE:} all'utente non è permesso creare due o più workflow con lo stesso nome.
	...
	\begin{figure}[!ht]
		\centering
		\fbox{\includegraphics[scale=0.2]{images/AddWorkflow.jpg}}
...
\begin{figure}[!ht]
	\centering
	\fbox{\includegraphics[scale=0.2]{images/Freccia.jpg}}
	\caption{Schermata aggiunta workflow}
\end{figure}
...
\begin{figure}[!ht]
	\centering
	\fbox{\includegraphics[scale=0.2]{images/ListBlocks.jpg}}
...
Questo blocco permette all'utente di inserire un qualsiasi feedRSS e successivamente far riprodurre al dispositivo\glossario{Alexa}il suo contenuto.
\begin{enumerate}
...
Lo scopo di questo blocco è quello di effettuare una sorta di autenticazione dell'utente per l'esecuzione di alcuni blocchi, ovvero: ReadMail, SendEmail e TwitterHashtag e TwitterUserTimeline.
\begin{enumerate}
...
L'utente inserendo questo blocco nel suo\glossario{workflow}potrà ascoltare gli ultimi post pubblicati da un utente che segue su Twitter.
...
\end{figure}
\subsection{Visualizzare blocchi nel workflow}\label{visualizzazione}
...
\subsubsection{Blocco TwitterHomeTimeline}
...
\newpage
    ...
    \begin{figure}[!ht]
    	...
    	\caption{Abilitazione Skill}
    \end{figure}
    \newpage
    \item Collegare l'account Amazan con la Skill: durante questo passaggio si verrà reindirizzati a una pagina di login di Amazon nella quale si dovranno inserire i dati relativi all'account Amazon che è in uso \\
...
\end{enumerate}
\newpage
\item  \textbf{Esecuzione skill MegAlex}\\
\\
 Per avviare la skill MegAlexa attraverso l'Echo Dot occorre pronunciare uno dei comandi vocali di avvio come ad esempio: "Alexa, open MegAlexa", oppurre "Alexa, start MegAlexa".
      
Se Alexa risponde con una richiesta della configurazione dell'account Amazon con la skill MegALexa allora  bisogna seguire le istruzioni indicate nella sezione precedente "configurazione Skill MegAlexa". 
