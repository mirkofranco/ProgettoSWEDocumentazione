...
\item Inserire il nome del nuovo\glossario{workflow}nell'apposito campo di testo;\\
...
L'utente inserendo questo blocco nel suo\glossario{workflow}potrà ascoltare i tweets di un altro utente a sua scelta tramite \textit{Alexa$_{G}$}.
\begin{enumerate}
...
Per poter utilizzare la \glossario{skill} \glossario{MegAlexa} è necessario configurare il proprio account Amazon con la skill  attraverso l'applicazione Amazon Alexa installata sul dispositivo mobile. Per fare ciò seguire i seguenti passi:
\begin{enumerate}
...
\newpage
    ...
    \end{figure}
    \newpage
    \item Collegare l'account Amazon con la \glossario{skill}: dopo aver eseguito il punto numero 4, si verrà reindirizzati a una pagina di login di Amazon nella quale si dovranno inserire i dati relativi all'\glossario{account} Amazon che è in uso; \\
    ...
    attraverso l'iterazione vocale utilizzando l'\glossario{Echo}.
    
\end{enumerate}
\newpage
\item  \textbf{Esecuzione skill MegAlexa}\label{Esecuzione skill}

Per avviare la skill MegAlexa attraverso l'\glossario{Echo} occorre pronunciare uno dei comandi vocali di avvio come ad esempio: "Alexa, open MegAlexa", oppure "Alexa, start MegAlexa".
...
A questo punto l'utente può avviare i \glossario{workflow} creati nell'\glossario{applicazione}.\\
