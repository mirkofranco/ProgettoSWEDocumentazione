...
\textbf{NOTA BENE:} all'utente non è permesso creare due o più workflow con lo stesso nome. É fortemente consigliato l'utilizzo delle lettere minuscole per la scrittura del nome (es. workflowTwitter). Inoltre si sconsiglia l'utilizzo di parole chiave all'interno del nome del workflow (es. english, Alexa,..);
...
\textbf{NOTA BENE:} la ricerca restituisce i primi 10 risultati che trova se non viene impostato alcun limite sul numero di tweets da cercare.
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Impostare il numero massimo di tweet da cercare (1-10) e poi cliccare su "Confirm".
	\end{enumerate}
	\item Cliccare su "Fatto".
\end{enumerate}

\subsubsection{Blocco TwitterUserTimeline}
L'utente inserendo questo blocco nel suo workflow potrà ascoltare i tweets di un altro utente a sua scelta tramite Alexa. \\
\textbf{NOTA BENE:} la ricerca restituisce i primi 10 risultati che trova se non viene impostato alcun limite sul numero di tweets da cercare.
\begin{enumerate}
	\item Nella schermata di elenco dei blocchi cliccare su "Twitter";
...
\subsubsection{Blocco TwitterWrite}
