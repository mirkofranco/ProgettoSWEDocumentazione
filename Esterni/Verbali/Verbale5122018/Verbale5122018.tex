...
\end{titlepage}
	...
	La riunione è stata convocata, attraverso l'invio di una mail all'azienda zero12, sia per avere un quadro più completo del capitolato, sia per ottenere delle delucidazioni su argomenti a noi poco chiari. \\
	...
	Per i primi minuti viene fatta un'introduzione al \textit{core} del progetto, durante i quali viene spiegato che dopo la registrazione avvenuta con successo da parte dell'utente, ci si aspetta che esso abbia la possibilità di creare dei \textit{workflow}, nei quali si darà la possibilità di scegliere un titolo e dei blocchi prefatti, per esempio:
	\begin{itemize}
	...
	Sì, verranno decisi assieme.
	
	\subsection{E' possibile sviluppare il progetto solamente in mobile?}
	Sì, anche se svilupparla per web è più semplice visto che basterebbe fare un'interfaccia web abbastanza responsive per averla a disposizione anche nel mobile
	...
	Volendo fare sia \textit{Android} che \textit{iOS} è importante che le due applicazioni non siano completamente sbilanciate, in quanto avere un'applicazione \textit{iOS} con poche funzionalità è abbastanza inutile e pertanto preferibile non farla.
	
	\subsection{Possibili moduli da implementare?}
	Moduli di base saranno:
	\begin{itemize}
	...
	Utilizzando l'account \textit{developer} di \textit{Alexa} è disponibile una sezione test dove poter interagire con l'assistente vocale, l'unica pecca è che non supporta il ritorno di un file audio. Inoltre sarà possibile svolgere i test con l'\textit{Amazon Echo}, per far ciò basterà prendere un appuntamento con l'azienda.
	...
	Questo argomento verrà spiegato più in dettaglio durante la lezione del 14 Dicembre. \\
	Dovrà essere incluso nel documento dell'\textbf{Analisi dei Requisiti}.
	
	\subsection{Schema di basi di dati da utilizzare?}
	Si consiglia di usare come \textit{NoSQL}: \textbf{dynamodb} piuttosto che \textbf{MongoDB}, entrambi utilizzano \textit{Jason}. Essendo entrambi i database documentali, si dovrà spiegare la struttura del documento che andremo a fare a supporto del progetto. \\
	Oppure si potrà valutare l'opzione di \textbf{Amazon Aurora Serverless} utilizzando quindi un \textit{MySQL}.
	
	\subsection{Documenti devono essere redatti in Inglese?}
	...
	Il piano di test di unità è principalmente per la parte server, ovvero bisognerà fare dei test per verificare che l'\textit{API} creata risponda alle richieste. Pertanto bisognerà utilizzare un approccio basato sui test. \\
	Inoltre nel blog di \textit{Amazon} sono presenti degli articoli per suggerire come fare i test in automatici sulle \textit{skills}, facendo attenzione che è un ambito abbastanza inesplorato. \\
