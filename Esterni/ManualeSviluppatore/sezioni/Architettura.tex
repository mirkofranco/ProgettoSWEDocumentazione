...
La\glossario{skill}è reperibile al seguente link: \url{https://developer.amazon.com/alexa/} (è necessario un account developer Amazon per accedervi). La skill è rappresentata da un JSON, configurabile attraverso l'UI fornito dalla piattaforma Alexa developer.
\begin{figure} [H]
    \centering
...
Le informazioni più importanti contenute nel file\glossario{JSON}sono:\label{paramsSkill}
\begin{itemize}
...
Il package services contiene la classe WorkflowService, che si occupa di creare un \textit{Workflow}, dopo aver ottenuto la sua rappresentazione\glossario{JSON}dal database (con una chiamata REST).
\begin{figure} [H]
    \centering
...
Il package blocks contiene tutti i blocchi della\glossario{skill}e il package utils.\\
...
\begin{itemize}
	...
	\item \textbf{Facade} utilizzato per le connessioni ad\glossario{API}esterne;
...
\begin{itemize}
	\item \textbf{convertToJSON(t: T):} utilizzato per la conversione delle informazioni ottenute in\glossario{JSON}e successivamente salvate in database;
